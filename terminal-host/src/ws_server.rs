...
pub fn build_router(state: AppState) -> Router {
    Router::new()
        ...
        .route("/ext-resource/{extension_id}/{*rel_path}", get(serve_ext_resource))
        .with_state(state)
}
...
async fn serve_ext_resource(Path((extension_id, rel_path)): Path<(String, String)>) -> impl IntoResponse {
    ...
    let (Ok(canonical_root), Ok(canonical_requested)) =
        (std::fs::canonicalize(&extension_dir), std::fs::canonicalize(&requested))
    else {
...
async fn handle_socket(socket: WebSocket, active_connections: Arc<AtomicUsize>, actual_port: u16) {
    ...
    while let Some(Ok(msg)) = ws_rx.next().await {
        ...
        match client_msg {
            ...
            ClientMessage::ExtHostInstall { extension_id, archive_base64 } => {
            ...
            ClientMessage::ExtHostActivate { extension_id, config, workspace_root } => {
                ...
                if !extension_dir.exists() {
                    let _ = out_tx.send(ServerMessage::ExtHostError {
                        extension_id,
                        message: "拡張機能がインストールされていません(先にインストールしてください)。".into(),
                    });
                    continue;
                }
                match ExtHostProcess::spawn(extension_id.clone(), extension_dir, actual_port, config, workspace_root, out_tx.clone()) {
                    Ok(process) => {
            ...
            ClientMessage::ExtHostConfigUpdate { extension_id, key, value } => {
                if let Some(process) = ext_hosts.get_mut(&extension_id) {
                    let _ = process.send_config_update(&key, value);
                }
            }
            ClientMessage::ExtHostExecuteCommand { extension_id, command, args } => {
                if let Some(process) = ext_hosts.get_mut(&extension_id) {
                    let _ = process.execute_command(&command, args);
                }
            }
            ClientMessage::ExtHostQuickPickResult { extension_id, request_id, selected_index } => {
                if let Some(process) = ext_hosts.get_mut(&extension_id) {
                    let _ = process.send_quick_pick_result(&request_id, selected_index);
                }
            }
            ClientMessage::ExtHostWebviewMessage { extension_id, view_id, message } => {
                if let Some(process) = ext_hosts.get_mut(&extension_id) {
                    let _ = process.send_webview_message(&view_id, message);
                }
            }
            ClientMessage::ExtHostWebviewVisibilityChanged { extension_id, view_id, visible } => {
                if let Some(process) = ext_hosts.get_mut(&extension_id) {
